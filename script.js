...
function showSong(songFilter){
	...
	// create image
        ...
        newImage.src = "assets/thumbnail/" + item.id + ".jpg";

	newImage.setAttribute("onerror","this.onerror=null; this.src='assets/nothumb.jpg'");
	...
	// creator
        let newCreator = document.createElement('a');
	...
	// Diffs
	let newDifficulties = document.createElement('div');
	newDifficulties.classList.add('difficulties');
        newItem.appendChild(newDifficulties);

	sortedDiffs = item.diffs.sort((a,b) => a.stars - b.stars).sort((a,b) => a.mode - b.mode);

	//console.log(sortedDiffs);

	sortedDiffs.forEach(function(message){
	    let newDiffs = document.createElement('img');
...
filterGenres.forEach(button => button.addEventListener("click", filterGeneros));
...
const filterExplicitos = (e) => {
    ...
    filterComplex();
    console.log("explicit filter=" + explicitFilter + " " + typeof explicitFilter)
    
}


filterExplicit.forEach(button => button.addEventListener("click", filterExplicitos));
